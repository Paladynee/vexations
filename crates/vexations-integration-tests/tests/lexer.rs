...
fn assert_lex_tokens(src: &str, expected_kinds: &[TK]) {
    ...
    let (tokens, spans, idents, errors) = lexer::lex(source.clone()).finalize();
...
fn logical_operators() {
    assert_lex_tokens("&& || !", &[TK::PuncAndAnd, TK::PuncOrOr, TK::PuncBang]);
}
...
fn validate_gen_output(
    src: VexationsSource, toks: &[TokenKind], spans: &[usize], idents: &[&str],
    errs: Vec<LexerError>, expected: &[TokenKind],
) {
    if !errs.is_empty() {
...
fn generated_test() {
    ...
    let (bytes, expected) = {
        let mut generator =
            LexerTestGenerator::new(AMOUNT_TOKENS_TEST, Some(TEST_SEED + 1));
